...
template <typename T>
struct EDOTOOLS_API is_bidirectional_iterator
...
template <typename T>
struct EDOTOOLS_API is_forward_iterator
      : public std::integral_constant<bool,
                                      (std::is_arithmetic<iterator_underlying_type_t<T>>::value ||
                                       std::is_compound<iterator_underlying_type_t<T>>::value) &&
                                          std::is_same<decltype(++(std::declval<T&>())), T&>::value> {};
...
template <typename T>
struct EDOTOOLS_API alignment_of : std::integral_constant<size_t, alignof(T)> {};

template <typename T>
struct EDOTOOLS_API size_of : std::integral_constant<size_t, sizeof(T)> {};
